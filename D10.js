...
delete me.age;
console.log(me.name + " ha " + me.age + " anni");
...
function isThisAnEmail(str90) {
  const email =
    /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
  return email.test(str90);
}

console.log(isThisAnEmail("bkff@gmail.com"));
...
const rollTheDices = (num85) => {
  let sum = 0;
  let values = [];

  for (let i = 0; i < num85; i++) {
    const value = dice();
    sum += value;
    values.push(value);
  }
...
// "16/12/2022" form data da inserire
console.log(isTodayMyBirthday("18/12/2022"));
...
const onlyTheYears = () => {
  ...
  for (let i = 0; i < movies.length; i++) {
    arrayWithYears.push(movies[i].Year);
  }
...
function removeIndex(movies, str78) {
  return [...movies.slice(0, str78), ...movies.slice(str78 + 1)];
}
...
const selectID = () => {
  const selezionoId = document.getElementById("container");
};
...
const addAnOtherElement = () => {
  const newRow = document.getElementById("myList");
